...
class GTSRBEnhanced(FashionMNIST):
    def __init__(
            self,
            root: str,
            split: str = "train",
        ...
        super(GTSRBEnhanced, self).__init__(root, split, transform, target_transform, download)
...
def load_data(dataset, data_path, device):
    if dataset == 'FashionMNIST':
        ...
        transform_normalize = transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize((0.3403), (0.2724)),
        ])

        data_train = GTSRBEnhanced(data_path,
                             transform=transform_normalize,
                             split = "train",
                             download=True, device=device)  # download=True for the first time
        data_test = GTSRBEnhanced(data_path,
                            split="test",
                            transform=transform_normalize, device=device)
    else:
